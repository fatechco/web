...
export const GiftCartPayment = ({
  ...
  const [fromWalletPrice, setFromWalletPrice] = useState<number | undefined>();
      ...
      createPaymentProcess(
        {
          ...
          data: {
            ...
            from_wallet_price: fromWalletPrice || undefined,
          },
        },
        {
          onSuccess: () => {
  ...
  const handleChangePayment = (value?: Payment) => {
    if (selectedPayment?.tag === "wallet" && fromWalletPrice) {
      setFromWalletPrice(undefined);
    }
    setSelectedPayment(value);
  };
  const handleChangeFromWalletPrice = (value?: number) => {
    setFromWalletPrice(value);
    if (!value) {
      handleChangePayment();
    }
  };
