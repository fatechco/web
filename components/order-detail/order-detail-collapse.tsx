...
const orderProgress: Record<string, { step: number; icon: React.ReactElement<any> }> = {
  new: {
    step: 1,
...
export const OrderDetailCollapse = ({ order, onRepeat }: OrderDetailCollapseProps) => {
  ...
  return (
    ...
    >
      ...
      <div className="px-2.5 py-4  bg-white bg-opacity-30 rounded-2xl flex flex-wrap items-center gap-2.5 my-4 w-full">
        <p>{t("ask.this.code.from.customer")}</p>
        <p>{order?.otp ?? t("N/A")}</p>
      </div>
